...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True)) as sess:
   ...
   #sess.run(model)
   saver = tf.train.import_meta_graph(LogDir + '/NN_tensorflow_fit.meta')
   ...
   with tf.device('/gpu:0'):
      ...
      for index in range(0,Nloop):
         ...
         for epoch in range(total_batch):
            ...
            batch = range(epoch*batch_size,epoch*batch_size+batch_size-1) #go thorugh enitre dataset
            ...
            if counter % 10 == 0:  # Record summaries and test-set accuracy
                ...
                if counter % record_step == (record_step-1):  # Record execution stats
                    ...
                    print("Batch #:", '%04d' % (epoch), "loss=", "{:.9f}".format(l))
