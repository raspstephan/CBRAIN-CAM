...
def prepare_dirs_and_logger(config):
    ...
    if config.load_path:
        ...
        print(4)
        config.model_name = get_time() + str(config.dataset) + '_layers_' + config.hidden + '_' + '_kdr_' + str(config.keep_dropout_rate)  + '_ac_' + str(config.act) + '_variables_' + config.input_names + config.addon#"{}_{}_{}".format(config.dataset, get_time(), ','.join(config.hidden.split(',')))
