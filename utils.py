...
def prepare_dirs_and_logger(config):
    ...
    if config.load_path:
        ...
        print(4)
        config.model_name = str(config.dataset) + '_' + get_time() + '_kdr_' + str(config.keep_dropout_rate)  + '_ac_' + str(config.act) + config.addon#"{}_{}_{}".format(config.dataset, get_time(), ','.join(config.hidden.split(',')))
