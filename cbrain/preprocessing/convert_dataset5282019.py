...
def create_stacked_da(ds, vars, SPCAM_v):
    """
    ...
    var_list, names_list = [], []
    for var in vars:
        if 'BP' in var and SPCAM_v!=5:
            da = compute_bp(ds, var)
        elif var in ['LHFLX', 'SHFLX']:
            da = ds[var][:-1]
        elif var == 'PRECST':
            da = (ds['PRECSC'] + ds['PRECSL'])[1:]
        elif var == 'PRECT' and SPCAM_v!=5:
            da = (ds['PRECC'] + ds['PRECL'])[1:]
#         elif var == 'PRECTEND' and SPCAM_v==5:
#             #da = (ds['QCTEND'])[1:]
#         elif var == 'PRECSTEN':
#             da = (ds['QITEND'])[1:]
        elif 'dt_adiabatic' in var and SPCAM_v!=5:
            base_var = var[:-12] + 'AP'
            da = compute_adiabatic(ds, base_var)
        elif var == 'PTEQ' and SPCAM_v==3:
            da = (ds['PHQ'])[1:]
        elif var == 'PTTEND' and SPCAM_v==3:
            da = (ds['TPHYSTND'])[1:]
        elif var == 'PHQ' and SPCAM_v==5:
            da = (ds['PTEQ'])[1:]
        elif var == 'TPHYSTND' and SPCAM_v==5:
            da = (ds['PTTEND'])[1:]
        else:
            da = ds[var][1:]
        var_list.append(da)
...
def preprocess(in_dir, in_fns, out_dir, out_fn, vars, SPCAM_v, lev_range=(0, 30)):
    """
    ...
    da = create_stacked_da(ds, vars, SPCAM_v)
